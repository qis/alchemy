...
namespace SKSE
{
	...
	bool Init(const LoadInterface* a_skse)
	{
		...
		if (!g_apiInit) {
			...
			if (g_pluginHandle == kPluginHandle_Invalid) {
				LOG_ERROR("Failed to get valid plugin handle!\n");
				return false;
			}
			...
			if (!g_scaleformInterface) {
				LOG_ERROR("Failed to query scaleform interface!\n");
				return false;
			}
			...
			if (!g_papyrusInterface) {
				LOG_ERROR("Failed to query papyrus interface!\n");
				return false;
			}
			...
			if (!g_messagingInterface) {
				LOG_ERROR("Failed to query messaging interface!\n");
				return false;
			} else {
				...
				if (!g_modCallbackEventSource) {
					LOG_ERROR("Failed to get mod callback event source!\n");
					return false;
				}
				...
				if (!g_cameraEventSource) {
					LOG_ERROR("Failed to get camera event source!\n");
					return false;
				}
				...
				if (!g_crosshairRefEventSource) {
					LOG_ERROR("Failed to get crosshair ref event source!\n");
					return false;
				}
				...
				if (!g_actionEventSource) {
					LOG_ERROR("Failed to get action event source!\n");
					return false;
				}
				...
				if (!g_niNodeUpdateEventSource) {
					LOG_ERROR("Failed to get ni node update event source!\n");
					return false;
				}
			}
	...
	bool AllocLocalTrampoline(std::size_t a_size)
	{
		if (!g_localTrampoline.Create(a_size)) {
			LOG_ERROR("Local trmapoline creation failed!\n");
			return false;
		}
	...
	bool AllocBranchTrampoline(std::size_t a_size)
	{
		if (!g_branchTrampoline.Create(a_size)) {
			LOG_ERROR("Branch trmapoline creation failed!\n");
			return false;
		}
