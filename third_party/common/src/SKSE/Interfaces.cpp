...
namespace SKSE
{
	...
	bool ScaleformInterface::Register(RegCallback* a_callback, const char* a_name) const
	{
		...
		if (!result) {
			LOG_ERROR("Failed to register %s for scaleform interface callback", a_name);
		}
		return result;
	}
	...
	bool PapyrusInterface::Register_Impl(RegFunction* a_fn) const
	{
		...
		if (vm) {
			...
			if (!result) {
				LOG_ERROR("Failed to register for papyrus interface");
			}
			return result;
		}
	}
	...
	bool MessagingInterface::Dispatch(UInt32 a_messageType, void* a_data, UInt32 a_dataLen, const char* a_receiver) const
	{
		...
		if (!result) {
			LOG_ERROR("Failed to dispatch message to %s", (a_receiver ? a_receiver : "all listeners"));
		}
		return result;
	}
	...
	bool MessagingInterface::RegisterListener(const char* a_sender, EventCallback* a_callback) const
	{
		...
		if (!result) {
			LOG_ERROR("Failed to register messaging listener for %s", a_sender);
		}
		return result;
	}
