...
{
	SKSE_FATALERROR("Assertion failed in %s (%d): %s", file, line, desc);
...
void _AssertionFailed_ErrCode(const char * file, unsigned long line, const char * desc, unsigned long long code)
{
	if(code & 0xFFFFFFFF00000000)
		SKSE_FATALERROR("Assertion failed in %s (%d): %s (code = %16I64X (%I64d))", file, line, desc, code, code);
	else
	{
		UInt32	code32 = code;
		SKSE_FATALERROR("Assertion failed in %s (%d): %s (code = %08X (%d))", file, line, desc, code32, code32);
	}
...
{
	SKSE_FATALERROR("Assertion failed in %s (%d): %s (code = %s)", file, line, desc, code);
