...
void ITimer::Init(void)
{
	if(!s_secondsPerCount)
	{
		...
		SKSE_MESSAGE("s_qpcWrapMargin: %016I64X", s_qpcWrapMargin);
		SKSE_MESSAGE("wrap time: %fs", ((double)0xFFFFFFFFFFFFFFFF) * s_secondsPerCount);

		// init multimedia timer
		timeGetDevCaps(&s_timecaps, sizeof(s_timecaps));

		SKSE_MESSAGE("min timer period = %d", s_timecaps.wPeriodMin);

		s_setTime = (timeBeginPeriod(s_timecaps.wPeriodMin) == TIMERR_NOERROR);
		if(!s_setTime)
			SKSE_WARNING("couldn't change timer period");
	}
}
...
void ITimer::DeInit(void)
{
	if(s_secondsPerCount)
	{
		...
		if(s_qpcWrapCount)
			SKSE_MESSAGE("s_qpcWrapCount: %d", s_qpcWrapCount);
