...
static bool DoInjectDLLThread(PROCESS_INFORMATION * info, const char * dllPath, bool sync, bool noTimeout)
{
	...
	if(process)
	{
		...
		if(hookBase)
		{
			...
			SKSE_MESSAGE("hookBase = %016I64X", hookBase);
			SKSE_MESSAGE("loadLibraryAAddr = %016I64X", loadLibraryAAddr);
			...
			if(thread)
			{
				if(sync)
				{
					...
					{
					case WAIT_OBJECT_0:
						SKSE_MESSAGE("hook thread complete");
						result = true;
						break;

					case WAIT_ABANDONED:
						SKSE_ERROR("Process::InstallHook: waiting for thread = WAIT_ABANDONED");
						break;

					case WAIT_TIMEOUT:
						SKSE_ERROR("Process::InstallHook: waiting for thread = WAIT_TIMEOUT");
						break;
					}
				}
				else
				...
				SKSE_ERROR("CreateRemoteThread failed (%d)", GetLastError());

			VirtualFreeEx(process, (LPVOID)hookBase, 0, MEM_RELEASE);
		}
		else
			SKSE_ERROR("Process::InstallHook: couldn't allocate memory in target process");

		CloseHandle(process);
	}
	else
		SKSE_ERROR("Process::InstallHook: couldn't get process handle");
	...
	bool	Init(PROCESS_INFORMATION * info, ProcHookInfo * hookInfo)
	{
		...
		if(m_proc)
		{
			...
			if(m_base)
			{
				...
				SKSE_MESSAGE("remote memory = %08X", m_base);
					...
					(bytesTransferred == sizeof(hookBaseCallAddr)))
				{
					...
					SKSE_MESSAGE("old winmain = %08X", hookBaseCallAddr);
					...
					{
						SKSE_ERROR("couldn't write memory (update winmain)");
					}
				}
				else
				{
					SKSE_ERROR("couldn't read memory (update winmain)");
				}
			}
			else
			{
				SKSE_ERROR("couldn't allocate memory in remote process");
			}
		}
		else
		{
			SKSE_ERROR("couldn't open process");
		}
...
static bool DoInjectDLL(PROCESS_INFORMATION * info, const char * dllPath, ProcHookInfo * hookInfo)
{
	...
	if(g_hookData.Init(info, hookInfo))
	{
		...
		{
			SKSE_ERROR("couldn't add library to list");
		}
	}
	else
	{
		SKSE_ERROR("couldn't init hook");
	}
