...
static bool GetFileVersion(const char * path, VS_FIXEDFILEINFO * info, std::string * outProductName, std::string * outProductVersion)
{
	...
	if(!versionSize)
	{
		SKSE_ERROR("GetFileVersionInfoSize failed (%08X)", GetLastError());
		return false;
	}
	...
	if(versionBuf)
	{
		if(GetFileVersionInfo(path, NULL, versionSize, versionBuf))
		{
			...
			{
				SKSE_ERROR("VerQueryValue failed (%08X)", GetLastError());
			}
		...
		{
			SKSE_ERROR("GetFileVersionInfo failed (%08X)", GetLastError());
		}
...
static bool GetFileVersionData(const char * path, UInt64 * out, std::string * outProductName)
{
	...
	SKSE_MESSAGE("dwSignature = %08X", versionInfo.dwSignature);
	SKSE_MESSAGE("dwStrucVersion = %08X", versionInfo.dwStrucVersion);
	SKSE_MESSAGE("dwFileVersionMS = %08X", versionInfo.dwFileVersionMS);
	SKSE_MESSAGE("dwFileVersionLS = %08X", versionInfo.dwFileVersionLS);
	SKSE_MESSAGE("dwProductVersionMS = %08X", versionInfo.dwProductVersionMS);
	SKSE_MESSAGE("dwProductVersionLS = %08X", versionInfo.dwProductVersionLS);
	SKSE_MESSAGE("dwFileFlagsMask = %08X", versionInfo.dwFileFlagsMask);
	SKSE_MESSAGE("dwFileFlags = %08X", versionInfo.dwFileFlags);
	SKSE_MESSAGE("dwFileOS = %08X", versionInfo.dwFileOS);
	SKSE_MESSAGE("dwFileType = %08X", versionInfo.dwFileType);
	SKSE_MESSAGE("dwFileSubtype = %08X", versionInfo.dwFileSubtype);
	SKSE_MESSAGE("dwFileDateMS = %08X", versionInfo.dwFileDateMS);
	SKSE_MESSAGE("dwFileDateLS = %08X", versionInfo.dwFileDateLS);
	SKSE_MESSAGE("productVersionStr = %s", productVersionStr.c_str());
...
bool ScanEXE(const char * path, ProcHookInfo * hookInfo)
{
	...
	{
		SKSE_ERROR("ScanEXE: couldn't open file (%d)", GetLastError());
		return false;
	}
	...
	if(mapping)
	{
		...
		{
			SKSE_ERROR("ScanEXE: couldn't map file (%d)", GetLastError());
		}
	...
	{
		SKSE_ERROR("ScanEXE: couldn't create file mapping (%d)", GetLastError());
	}
...
bool IdentifyEXE(const char * procName, bool isEditor, std::string * dllSuffix, ProcHookInfo * hookInfo)
{
	...
	SKSE_MESSAGE("version = %016I64X", version);
	SKSE_MESSAGE("product name = %s", productName.c_str());

	if(productName == "SKSE64")
	{
		SKSE_MESSAGE("found an SKSE64 component");
		return false;
	}
	...
	{
	case kProcType_Steam:	SKSE_MESSAGE("steam exe"); break;
	case kProcType_Normal:	SKSE_MESSAGE("normal exe"); break;
	case kProcType_Packed:	SKSE_MESSAGE("packed exe"); break;
	case kProcType_Unknown:
	default:				SKSE_MESSAGE("unknown exe type"); break;
	}
