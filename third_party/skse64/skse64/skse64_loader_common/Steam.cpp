...
std::string GetRegKey(HKEY root, const char * path, char * value)
{
	...
	if(!err)
	{
		...
		if(!err)
		{
			...
			{
				SKSE_ERROR("GetRegKey: error getting value (%08X)", err);
			}
		}
		else
		{
			SKSE_ERROR("GetRegKey: error querying value length (%08X)", err);
		}
	...
	{
		SKSE_ERROR("GetRegKey: registry key not found (%08X)", err);
	}
...
bool SteamCheckPassive(void)
{
	...
	if(snap != INVALID_HANDLE_VALUE)
	{
		...
		{
			SKSE_ERROR("SteamCheck: Module32First failed (%d)", GetLastError());
			result = true;
		}
	...
	{
		SKSE_ERROR("SteamCheck: CreateToolhelp32Snapshot failed (%d)", GetLastError());
...
static bool SteamCheckActive(void)
{
	...
	if(steamAPI)
	{
		...
		if(SteamAPI_IsSteamRunning)
		{
			...
			if(!result)
			{
				SKSE_ERROR("timed out waiting for steam boot");
			}
		}
		else
		{
			SKSE_ERROR("couldn't get steam API ptr");
		}
	...
	{
		SKSE_ERROR("couldn't load steam API DLL (%08X)", GetLastError());
	}
...
bool SteamLaunch(void)
{
	...
	SKSE_MESSAGE("steam root = %s", steamRoot.c_str());
