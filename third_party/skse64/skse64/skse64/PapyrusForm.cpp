	...
	TESForm * TempClone(TESForm * thisForm)
	{
		...
		if(thisForm)
		{
			...
			if(factory)
			{
				...
				{
					SKSE_ERROR("Form::TempClone: factory for type %02X failed", thisForm->formType);
				}
			}
			else
			{
				SKSE_MESSAGE("Form::TempClone: tried to clone form %08X (type %02X), no factory found", thisForm->formID, thisForm->formType);
			}
		}
