...
public:
	...
	void	Dump(void)
	{
		...
		{
			SKSE_MESSAGE("* %d :", i);
			//SKSE_MESSAGE("\t\titem: %08X", pCur->item);
			if (pCur->item)
				SKSE_MESSAGE("\t\t*item: %d", *pCur->item);
			SKSE_MESSAGE("\t\tnext: %08X", pCur->next);
...
public:
	...
	void Dump(void)
	{
		SKSE_MESSAGE("tHashSet:");
		SKSE_MESSAGE("> size: %d", Size());
		SKSE_MESSAGE("> free: %d", FreeCount());
		SKSE_MESSAGE("> filled: %d", FillCount());
		if (m_entries)
		{
			_Entry * p = m_entries;
			for (UInt32 i = 0; i < m_size; i++, p++) {
				SKSE_MESSAGE("* %d %s:", i, p->IsFree()?"(free)" : "");
				if (!p->IsFree())
					p->Dump();
			}
		}
