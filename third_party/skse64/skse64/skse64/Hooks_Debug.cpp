...
LONG WINAPI ExceptionFilter(EXCEPTION_POINTERS * info)
{
	if(s_inExceptionFilter.Claim())
	{
		...
		if(crashReason)
		{
			SKSE_ERROR("Skyrim has crashed in a known crash location (%s). No crashdump will be written in release builds.", crashReason);
		...
		if(writeDump)
		{
			SKSE_ERROR("Skyrim has crashed. A minidump containing debugging information is being written to %s.", s_crashDumpPath);
			...
			if(dumpFile != INVALID_HANDLE_VALUE)
			{
				...
				if(result)
				{
					SKSE_ERROR("Minidump written.");
				}
				else
				{
					SKSE_ERROR("Unable to write minidump.");
				}
			...
			{
				SKSE_ERROR("Unable to open minidump. (%08X)", GetLastError());
			}
		}
	...
	{
		SKSE_ERROR("top-level exception filter hit multiple times");
	}
...
{
	SKSE_MESSAGE("SetUnhandledExceptionFilter_Hook: %p", lpTopLevelExceptionFilter);
...
void Hooks_Debug_Init(void)
{
	...
	if(enableMiniDump)
	{
		SKSE_MESSAGE("minidumps enabled");
		...
		if(s_dbgHelpDLL)
		{
			...
			if(!s_dbgHelpWriteDump)
				SKSE_WARNING("dbghelp missing MiniDumpWriteDump, upgrade to dbghelp 5.1 or later");
		}
		else
		{
			SKSE_MESSAGE("no dbghelp");
		}
		...
		if(s_dbgHelpDLL && s_dbgHelpWriteDump)
		{
			...
			SKSE_MESSAGE("old exception filter = %p", s_oldExceptionFilter);
