...
SKSE_MESSAGE("TRI Database...");
//DumpClass(triDatabase, sizeof(FaceGenTriDatabase)/4);
gLog.Indent();
BSFaceGenDB::TRI::DBTraits::Data * triEntry;
SKSE_MESSAGE("Database Capacity: %d", triDatabase->data.triEntries.arr.capacity);
for(UInt32 i = 0; i < triDatabase->data.triEntries.arr.capacity; i++) {
	...
	if(triEntry && triEntry != FaceGenTriDatabase::GetInvalidEntry()) {
		SKSE_MESSAGE("Tri Entry %d", i);
		//DumpClass(triEntry, sizeof(BSFaceGenDB::TRI::DBTraits::Data)/4);
		if(triEntry->morphSet) {
			//SKSE_MESSAGE("Set %d - %s", i, triEntry->morphSet->fileName);
			//DumpClass(triEntry->morphSet, sizeof(BSFaceGenDB::TRI::DBTraits::MorphSet)/4);
			SKSE_MESSAGE("Set - %s MorphCount: %d", triEntry->morphSet->fileName, triEntry->morphSet->morphData.count);
			for(UInt32 n = 0; n < triEntry->morphSet->morphData.count; n++) {
				...
				if(triEntry->morphSet->morphData.GetNthItem(n, morphEntry)) {
					//SKSE_MESSAGE("Morph %d %d - %s", i, n, morphEntry.morphName);
					//DumpClass(&morphEntry, sizeof(BSFaceGenDB::TRI::DBTraits::MorphData)/4);
					SKSE_MESSAGE("Morph %d %d - %s baseDiff: %f DiffVertNum: %d BaseVertNum: %d DiffVertPosNum: %d", i, n, morphEntry.morphName, morphEntry.baseDiff, morphEntry.diffVertexNum, morphEntry.baseVertexNum, morphEntry.diffVertexPosNum);
				}
			}
			...
			while(children) {
				SKSE_MESSAGE("Child Set - %s MorphCount: %d", children->morphSet->fileName, children->morphSet->morphData.count);
				gLog.Indent();
				//SKSE_MESSAGE("Child Set %d - %s", i, children->morphSet->fileName);
				...
				for(UInt32 n = 0; n < children->morphSet->morphData.count; n++) {
					...
					if(children->morphSet->morphData.GetNthItem(n, morphEntry)) {
						//SKSE_MESSAGE("Child Morph %d %d - %s", k, n, morphEntry.morphName);
						//DumpClass(&morphEntry, sizeof(BSFaceGenDB::TRI::DBTraits::MorphData)/4);
						SKSE_MESSAGE("Child Morph %d %d - %s baseDiff: %f DiffVertNum: %d BaseVertNum: %d DiffVertPosNum: %d", i, n, morphEntry.morphName, morphEntry.baseDiff, morphEntry.diffVertexNum, morphEntry.baseVertexNum, morphEntry.diffVertexPosNum);
					}
				}
				gLog.Outdent();
				k++;
		...
		SKSE_MESSAGE("Invalid entry %d", i);
	}
}
