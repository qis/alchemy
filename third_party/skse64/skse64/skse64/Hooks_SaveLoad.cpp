...
#ifdef DEBUG
	SKSE_MESSAGE("Executing BGSSaveLoadManager::SaveGame_Hook. saveName: %s", saveName);
#endif
...
#ifdef DEBUG
	SKSE_MESSAGE("Executed BGSSaveLoadManager::SaveGame_Hook.");
#endif
}
...
bool BGSSaveLoadManager::LoadGame_Hook(UInt64 *unk0, UInt32 unk1, UInt32 unk2, void *unk3)
{
...
#ifdef DEBUG
	SKSE_MESSAGE("Executing BGSSaveLoadManager::LoadGame_Hook. saveName: %s", saveName);
#endif
	...
	if(GetConfigOption_UInt32("General", "ClearInvalidRegistrations", &enableClearRegs))
	{
		if(enableClearRegs)
		{
			...
			if (count > 0)
				SKSE_MESSAGE("ClearInvalidRegistrations: Removed %d invalid OnUpdate registration(s)", count);
		}
	}

#ifdef DEBUG
	SKSE_MESSAGE("Executed BGSSaveLoadManager::LoadGame_Hook.");
#endif
...
void BGSSaveLoadManager::ProcessEvents_Hook(void)
{
	...
	if(s_requestedSave && s_requestedLoad)
		SKSE_MESSAGE("BGSSaveLoadManager: save and load requested in the same frame, ignoring both");
