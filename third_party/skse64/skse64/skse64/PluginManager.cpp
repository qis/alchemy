...
bool PluginManager::Init(void)
{
	...
	{
		SKSE_MESSAGE("plugin directory = %s", m_pluginDirectory.c_str());
		...
		__except(EXCEPTION_EXECUTE_HANDLER)
		{
			...
			SKSE_ERROR("exception occurred while loading plugins");
		}
	}
...
void * PluginManager::QueryInterface(UInt32 id)
{
	...
	default:
		SKSE_WARNING("unknown QueryInterface %08X", id);
		break;
	}
#else
	SKSE_WARNING("unknown QueryInterface %08X", id);
...
void PluginManager::InstallPlugins(void)
{
	...
	for(IDirectoryIterator iter(m_pluginDirectory.c_str(), "*.dll"); !iter.Done(); iter.Next())
	{
		...
		SKSE_MESSAGE("checking plugin %s", pluginPath.c_str());
		...
		if(plugin.handle)
		{
			...
			if(plugin.query && plugin.load)
			{
				...
				SKSE_MESSAGE("plugin %s (%08X %s %08X) %s",
					pluginPath.c_str(),
			...
			{
				SKSE_MESSAGE("plugin %s does not appear to be an SKSE plugin", pluginPath.c_str());
			}
		...
		{
			SKSE_ERROR("couldn't load plugin %s (Error %d)", pluginPath.c_str(), GetLastError());
		}
	}
...
void * PluginManager::GetEventDispatcher(UInt32 dispatcherId)
{
	...
	default:
		SKSE_WARNING("unknown EventDispatcher %08X", dispatcherId);
		break;
	}
#else
	SKSE_WARNING("unknown EventDispatcher %08X", id);
#endif
...
bool PluginManager::RegisterListener(PluginHandle listener, const char* sender, SKSEMessagingInterface::EventCallback handler)
{
	...
	SKSE_MESSAGE("registering plugin listener for %s at %u of %u", sender, listener, numPlugins);
...
bool PluginManager::Dispatch_Message(PluginHandle sender, UInt32 messageType, void * data, UInt32 dataLen, const char* receiver)
{
	...
	for (std::vector<PluginListener>::iterator iter = s_pluginListeners[sender].begin(); iter != s_pluginListeners[sender].end(); ++iter)
	{
		...
		{
			SKSE_DMESSAGE("sending message type %u to plugin %u", messageType, iter->listener);
			iter->handleMessage(&msg);
			numRespondents++;
		}
	}
	SKSE_DMESSAGE("dispatched message.");
