...
void SKSE64_Initialize(void)
{
	...
	__try {
		...
		SKSE_MESSAGE("SKSE64 runtime: initialize (version = %d.%d.%d %08X %08X%08X, os = %s)",
		...
		SKSE_MESSAGE("imagebase = %016I64X", GetModuleHandle(NULL));
		SKSE_MESSAGE("reloc mgr imagebase = %016I64X", RelocationManager::s_baseAddr);
		...
		{
			SKSE_ERROR("couldn't create branch trampoline. this is fatal. skipping remainder of init process.");
			return;
		}

		if(!g_localTrampoline.Create(1024 * 64, g_moduleHandle))
		{
			SKSE_ERROR("couldn't create codegen buffer. this is fatal. skipping remainder of init process.");
			return;
		}
	...
	{
		SKSE_ERROR("exception thrown during startup");
	}
#endif

	SKSE_MESSAGE("init complete");
}
