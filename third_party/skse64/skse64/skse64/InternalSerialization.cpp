...
void LoadModList(SKSESerializationInterface * intfc)
{
	...
	for (UInt32 i = 0; i < numSavedMods; i++)
	{
		...
		if (modInfo) {
			...
			SKSE_MESSAGE("\t(%d -> %d)\t%s", i, newIndex, &name);
		}
		else {
...
void SavePluginsList(SKSESerializationInterface * intfc)
{
	...
	SKSE_MESSAGE("Saving plugin list:");
	...
	dhand->modList.modInfoList.Visit(LoadedModVisitor([&](ModInfo* modInfo)
	{
		if (modInfo && modInfo->IsActive())
		{
			...
			if (modInfo->modIndex != 0xFE)
			{
				SKSE_MESSAGE("\t[%d]\t%s", modInfo->modIndex, &modInfo->name);
			}
			else
			{
				SKSE_MESSAGE("\t[FE:%d]\t%s", modInfo->lightIndex, &modInfo->name);
			}
		}
...
void LoadPluginList(SKSESerializationInterface * intfc)
{
	...
	SKSE_MESSAGE("Loading plugin list:");
	...
	for (UInt32 i = 0; i < modCount; i++)
	{
		...
		SKSE_MESSAGE("\t(%d -> %d)\t%s", oldIndex, newIndex, name);
	}
}
...
{
	SKSE_MESSAGE("Loading light mod list:");
	...
	for (UInt32 i = 0; i < numSavedMods; i++)
	{
		...
		if (modInfo) {
			...
			SKSE_MESSAGE("\t(%d -> %d)\t%s", lightIndex, newIndex, &name);
		}
		else {
			s_savedModIndexMap[lightIndex] = 0xFF;
		}
		SKSE_MESSAGE("\t(%d -> %d)\t%s", lightIndex, s_savedModIndexMap[lightIndex], &name);
	}
}
...
void Core_SaveCallback(SKSESerializationInterface * intfc)
{
	...
	SKSE_MESSAGE("Saving menu open/close event registrations...");
	g_menuOpenCloseRegs.Save(intfc, 'MENR', 1);

	SKSE_MESSAGE("Saving key input event registrations...");
	g_inputKeyEventRegs.Save(intfc, 'KEYR', 1);

	SKSE_MESSAGE("Saving control input event registrations...");
	g_inputControlEventRegs.Save(intfc, 'CTLR', 1);

	SKSE_MESSAGE("Saving mod callback event registrations...");
	g_modCallbackRegs.Save(intfc, 'MCBR', 1);

	SKSE_MESSAGE("Saving crosshair ref event registrations...");
	g_crosshairRefEventRegs.Save(intfc, 'CHRR', 1);

	SKSE_MESSAGE("Saving camera event registrations...");
	g_cameraEventRegs.Save(intfc, 'CAMR', 1);

	SKSE_MESSAGE("Saving actor action event registrations...");
	g_actionEventRegs.Save(intfc, 'AACT', 1);

	SKSE_MESSAGE("Saving NiNode update event registrations...");
	g_ninodeUpdateEventRegs.Save(intfc, 'NINU', 1);

	SKSE_MESSAGE("Saving SKSEPersistentObjectStorage data...");
	SaveClassHelper(intfc, 'OBMG', SKSEObjectStorageInstance());

	SKSE_MESSAGE("Saving SKSEDelayFunctorManager data...");
...
void Core_LoadCallback(SKSESerializationInterface * intfc)
{
	...
	while (intfc->GetNextRecordInfo(&type, &version, &length))
	{
		...
		case 'MENR':
			SKSE_MESSAGE("Loading menu open/close event registrations...");
			g_menuOpenCloseRegs.Load(intfc, 1);
			break;

		// Key input events
		case 'KEYR':
			SKSE_MESSAGE("Loading key input event registrations...");
			g_inputKeyEventRegs.Load(intfc, 1);
			break;

		// Control input events
		case 'CTLR':
			SKSE_MESSAGE("Loading control input event registrations...");
			g_inputControlEventRegs.Load(intfc, 1);
			break;

		// Custom mod events
		case 'MCBR':
			SKSE_MESSAGE("Loading mod callback event registrations...");
			g_modCallbackRegs.Load(intfc, 1);
			break;

		// Crosshair rev events
		case 'CHRR':
			SKSE_MESSAGE("Loading crosshair ref event registrations...");
			g_crosshairRefEventRegs.Load(intfc, 1);
			break;

		// Camera events
		case 'CAMR':
			SKSE_MESSAGE("Loading camera event registrations...");
			g_cameraEventRegs.Load(intfc, 1);
			break;

		// Actor Actions events
		case 'AACT':
			SKSE_MESSAGE("Loading actor action event registrations...");
			g_actionEventRegs.Load(intfc, 1);
			break;

		// NiNode update events
		case 'NINU':
			SKSE_MESSAGE("Loading NiNode update event registrations...");
			g_ninodeUpdateEventRegs.Load(intfc, 1);
			break;

		// SKSEPersistentObjectStorage
		case 'OBMG':
			SKSE_MESSAGE("Loading SKSEPersistentObjectStorage data...");
			SKSEObjectStorageInstance().Load(intfc, version);
			break;

		// SKSEDelayFunctorManager
		case 'DFMG':
			SKSE_MESSAGE("Loading SKSEDelayFunctorManager data...");
			SKSEDelayFunctorManagerInstance().Load(intfc, version);
			break;

		default:
			SKSE_MESSAGE("Unhandled chunk type in Core_LoadCallback: %08X (%.4s)", type, &type);
			continue;
		}
	}
}
