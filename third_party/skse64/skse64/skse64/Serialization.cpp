...
namespace Serialization
{
	...
	void SetUniqueID(PluginHandle plugin, UInt32 uid)
	{
		...
		for(PluginCallbackList::iterator iter = s_pluginCallbacks.begin(); iter != s_pluginCallbacks.end(); ++iter)
		{
			if(iter->hadUID && (iter->uid == uid))
			{
				...
				SKSE_ERROR("plugin serialization UID collision (uid = %08X, plugins = %d %d)", plugin, uid, collidingID);
			}
		}
	...
	void SetSaveName(const char * name)
	{
		if(name)
		{
			...
			SKSE_MESSAGE("save name is %s", save_name.c_str());
			s_savePath = MakeSavePath(save_name, ".skse");
			SKSE_MESSAGE("full save path: %s", s_savePath.c_str());
		}
		else
		{
			SKSE_MESSAGE("cleared save path");
			s_savePath.clear();
		}
	}
	...
	static void FlushReadRecord(void)
	{
		if(s_chunkOpen)
		{
			...
			{
				// SKSE_WARNING("plugin didn't finish reading chunk");
	...
	{
		SKSE_MESSAGE("creating co-save");
		if(!s_currentFile.Create(s_savePath.c_str()))
		{
			SKSE_ERROR("HandleSaveGlobalData: couldn't create save file (%s)", s_savePath.c_str());
			return;
		}
		...
		try
		{
			...
			for(UInt32 i = 0; i < s_pluginCallbacks.size(); i++)
			{
				...
				if(info->save && info->hadUID)
				{
					...
					{
						SKSE_ERROR("HandleSaveGlobalData: exception occurred saving %08X at %016I64X data may be corrupt.", s_pluginHeader.signature, s_currentFile.GetOffset());
					}
		...
		{
			SKSE_ERROR("HandleSaveGame: exception during save");
		}
	...
	void HandleLoadGlobalData(void)
	{
		...
		try
		{
			...
			{
				SKSE_ERROR("HandleLoadGame: invalid file signature (found %08X expected %08X)", header.signature, Header::kSignature);
				goto done;
			}

			if(header.formatVersion <= Header::kVersion_Invalid)
			{
				SKSE_ERROR("HandleLoadGame: version invalid (%08X)", header.formatVersion);
				goto done;
			}

			if(header.formatVersion > Header::kVersion)
			{
				SKSE_ERROR("HandleLoadGame: version too new (found %08X current %08X)", header.formatVersion, Header::kVersion);
				goto done;
			}
			...
			while(s_currentFile.GetRemain() >= sizeof(PluginHeader))
			{
				...
				try
				{
					...
					{
						SKSE_WARNING("HandleLoadGame: plugin with signature %08X not loaded", s_pluginHeader.signature);
					}
				}
				catch( ... )
				{
					SKSE_ERROR("HandleLoadGame: exception occurred loading %08X", s_pluginHeader.signature);
				}
				...
				{
					SKSE_WARNING("HandleLoadGame: plugin did not read all of its data (at %016I64X expected %016I64X)", s_currentFile.GetOffset(), expectedOffset);
		...
		{
			SKSE_ERROR("HandleLoadGame: exception during load");
	...
	void HandleDeleteSave(std::string saveName)
	{
		...
		{
			SKSE_MESSAGE("deleting co-save %s", coSavePath.c_str());	
			DeleteFile(coSavePath.c_str());
		}
		else
		{
			SKSE_MESSAGE("skipped delete of co-save %s", coSavePath.c_str());	
		}
	}
