...
void SKSEPersistentObjectStorage::CleanDroppedStacks()
{
	...
	for (UInt32 i=0; i<data_.size(); i++)
	{
		...
		SKSE_MESSAGE("SKSEPersistentObjectStorage::CleanDroppedStacks: Freed object at index %d.", i);
	}
}
...
ISKSEObject* SKSEPersistentObjectStorage::Access(SInt32 handle)
{
	...
	{
		SKSE_MESSAGE("SKSEPersistentObjectStorage::AccessObject(%d): Invalid handle.", handle);
		return NULL;
	}
	...
	if (e.obj == NULL)
	{
		SKSE_MESSAGE("SKSEPersistentObjectStorage::AccessObject(%d): Object was NULL.", handle);
		return NULL;
	}
	...
	if (result == NULL)
	{
		SKSE_MESSAGE("SKSEPersistentObjectStorage::AccessObject(%d): Invalid type (%s).", handle, e.obj->ClassName());
		return NULL;
	}
...
ISKSEObject* SKSEPersistentObjectStorage::Take(SInt32 handle)
{
	...
	if (e.obj == NULL)
	{
		SKSE_MESSAGE("SKSEPersistentObjectStorage::TakeObject(%d): Object was NULL.", handle);
		return NULL;
	}
	...
	{
		SKSE_MESSAGE("SKSEPersistentObjectStorage::TakeObject(%d): Invalid type (%s).", handle, e.obj->ClassName());
		return NULL;
	}
...
bool ReadSKSEObject(SKSESerializationInterface* intfc, ISKSEObject*& objOut)
{
	...
	if (type != 'OBJE')
	{
		SKSE_MESSAGE("ReadSKSEObject: Error loading unexpected chunk type %08X (%.4s)", type, &type);
		return false;
	}
	...
	if (len > kMaxNameLen)
	{
		SKSE_MESSAGE("ReadSKSEObject: Serialization error. Class name len extended kMaxNameLen.");
		return false;
	}
	...
	if (factory == NULL)
	{
		SKSE_MESSAGE("ReadSKSEObject: Serialization error. Factory missing for %s.", &buf);
		return false;
	}
