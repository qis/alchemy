...
public:
	...
	void Dump(void)
	{
		SKSE_MESSAGE("> handle:\t%016I64x", handle);
		params.Dump();
	}
};
...
public:
	...
	void Dump(void)
	{
		SKSE_MESSAGE("> callbackName:\t%s", callbackName);
	}
};
...
#ifdef _DEBUG
		SKSE_MESSAGE("Executed PapyrusEvents::Register - %016llX", reg.handle);
#endif
		...
		while (intfc->GetNextRecordInfo(&type, &curVersion, &length))
		{
			switch (type)
			{
				case 'REGS':
				{
					...
					{
						SKSE_MESSAGE("Error loading reg key");
						return false;
					}
					...
					{
						SKSE_MESSAGE("Error loading reg count");
						return false;
					}
					...
					for (UInt32 i=0; i<numRegs; i++)
					{
						...
						{
							SKSE_MESSAGE("Error loading regs");
							return false;
						}
					}
				...
				default:
				{
					SKSE_MESSAGE("Error loading unexpected chunk type %08X (%.4s)", type, &type);
					return false;
				}
			}
		}

		SKSE_MESSAGE("Missing record data for type %08X (%.4s)", type, &type);
		return false;
	}
};
