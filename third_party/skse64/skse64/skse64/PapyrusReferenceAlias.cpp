	...
	bool ForceRefToWornItem(BGSRefAlias * alias, TESObjectREFR * reference, UInt32 mask)
	{
		...
		if (pContainerChanges) {
			...
			if(xReference) { // Persistent Ref exists, acquire it
				...
				SKSE_MESSAGE("Found Reference %08X", foundReference->formID);
				...
				if(foundReference) {
					...
					SKSE_MESSAGE("Created Reference %08X", foundReference->formID);
				}*/
			}

			if(foundReference)
			{
				SKSE_MESSAGE("Forced Reference to %08X", foundReference->formID);
				CALL_MEMBER_FN(alias->owner, ForceRefTo)(alias->aliasId, foundReference);
				return true;
			}
			else
			{
				SKSE_MESSAGE("Forced Reference to <not found>");
			}
		}
