...
bool RegisterScaleformPlugin(const char * name, SKSEScaleformInterface::RegisterCallback callback)
{
	...
	for(PluginList::iterator iter = s_plugins.begin(); iter != s_plugins.end(); ++iter)
	{
		...
		{
			SKSE_WARNING("scaleform plugin name collision: %s", iter->name);
			return false;
		}
	}
...
#if _DEBUG
			SKSE_MESSAGE("scaleform: AllowTextInput (counter = %d)", allowCount);
#endif
		}
		else
			SKSE_WARNING("input manager not initialized");
	}
};
...
public:
	virtual void	Invoke(Args * args)
	{
...
#if _DEBUG
		SKSE_MESSAGE("scaleform: SetINISetting %s %f", settingName, settingValue);
#endif
		...
		if(setting)
		{
			...
			{
				SKSE_MESSAGE("tried to set a non-floating-point setting (%s)", settingName);
			}
		}
		else
		{
			SKSE_MESSAGE("setting not found (%s)", settingName);
		}
	}
};
...
public:
	virtual void	Invoke(Args * args)
	{
...
#if _DEBUG
		SKSE_MESSAGE("scaleform: GetINISetting %s", settingName);
#endif
		...
		if(setting)
		{
			...
			{
				SKSE_MESSAGE("tried to get a non-floating-point setting (%s)", settingName);
			}
			else
			{
...
#if _DEBUG
		SKSE_MESSAGE("scaleform: open menu (%s)", menuName);
#endif
...
#if _DEBUG
		SKSE_MESSAGE("scaleform: close menu (%s)", menuName);
#endif
...
#if _DEBUG
		SKSE_MESSAGE("scaleform: send mod event (%s, %s, %d)", eventName, strArg, numArg);
#endif
...
public:
	virtual void	Invoke(Args * args)
	{
		...
		if(!formArg) {
			SKSE_MESSAGE("SKSEScaleform_RequestActorValues - Invalid form (%X)", formidArg);
			g_loadGameLock.Leave();
			return;
		}
		...
		if(!actor) {
			SKSE_MESSAGE("SKSEScaleform_RequestActorValues - Invalid form type (%X)", formidArg);
			g_loadGameLock.Leave();
			return;
		}
...
//	SKSE_MESSAGE("StandardItemData hook");
...
//	SKSE_MESSAGE("MagicItemData hook");
...
namespace GFxLoaderHook
{
	...
	GFxLoader *ctor_Hook(GFxLoader * loader)
	{
		...
		//	SKSE_MESSAGE("result == singleton %d", (UInt32)(result == GetSingleton()));
...
//	SKSE_MESSAGE("got new scaleform menu (view = %p)", view);
	...
	if(!result)
	{
		SKSE_ERROR("couldn't get _global");
		return;
	}
