...
const std::string & GetRuntimeDirectory()
{
	...
	if(s_runtimeDirectory.empty())
	{
		...
		{
			SKSE_WARNING("no slash in runtime path? (%s)", runtimePath.c_str());
		}
	}
...
const std::string & GetConfigPath()
{
	...
	if(s_configPath.empty())
	{
		...
		if(!runtimePath.empty())
		{
			...
			SKSE_MESSAGE("config path = %s", s_configPath.c_str());
		}
	}
...
void DumpClass(void * theClassPtr, UInt64 nIntsToDump)
{
	...
	SKSE_MESSAGE("DumpClass: %016I64X", basePtr);
	...
	for (UInt64 ix = 0; ix < nIntsToDump; ix++ ) {
		...
		SKSE_MESSAGE("%3d +%03X ptr: 0x%016I64X: %32s *ptr: 0x%016I64x | %f, %f: %32s", ix, ix*8, curPtr, curPtrName, otherPtr, otherFloat2, otherFloat1, otherPtrName);
	}
