...
bool BranchTrampoline::Create(size_t len, void * module)
{
	...
	while(!m_base)
	{
		...
		{
			SKSE_ERROR("VirtualQuery failed: %08X", GetLastError());
			break;
		}
		...
		if(info.State == MEM_FREE)
		{
			...
			if(info.RegionSize >= len)
			{
				...
				{
					SKSE_WARNING("trampoline alloc %016I64Xx%016I64X failed (%08X)", addr, len, GetLastError());
				}
			}
		}
		...
		{
			SKSE_ERROR("couldn't allocate trampoline, no free space before image");
			break;
		}
	}
