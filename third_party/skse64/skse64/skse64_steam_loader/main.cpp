...
static void OnAttach(void)
{
	...
	SKSE_MESSAGE("skse64 loader %08X (steam) %08X%08X %s", PACKED_SKSE_VERSION, now.dwHighDateTime, now.dwLowDateTime, GetOSInfoStr().c_str());
	SKSE_MESSAGE("loader base addr = %016I64X", g_dllHandle);
	SKSE_MESSAGE("exe base addr = %016I64X", GetModuleHandle(NULL));
...
static void HookIAT()
{
	...
	if(iat)
	{
		SKSE_MESSAGE("found iat at %016I64X", iat);

		__telemetry_main_invoke_trigger_Original = *iat;
		SKSE_MESSAGE("original thunk %016I64X", __telemetry_main_invoke_trigger_Original);

		SafeWrite64(uintptr_t(iat), (UInt64)__telemetry_main_invoke_trigger_Hook);
		SKSE_MESSAGE("patched iat");
	}
	else
	{
		SKSE_MESSAGE("couldn't find __telemetry_main_invoke_trigger");
	}
}
...
static void HookMain(void * retAddr)
{
	...
	SKSE_MESSAGE("HookMain: thread = %d retaddr = %016I64X", GetCurrentThreadId(), retAddr);

	std::string runtimePath = GetRuntimePath();
	SKSE_MESSAGE("runtimePath = %s", runtimePath.c_str());
	...
	SKSE_MESSAGE("dll = %s", g_dllPath.c_str());
